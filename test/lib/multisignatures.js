...
var NoEbookcoinAccount = node.randomAccount();
NoEbookcoinAccount.name = "noebookcoin";
...
function openAccount (account, i){
        node.api.post('/accounts/open')
            ...
            .end(function (err, res) {
                ...
                else if (account.name == "noebookcoin"){
                    NoEbookcoinAccount.address = res.body.account.address;
                    NoEbookcoinAccount.publicKey = res.body.account.publicKey;
                }
...
// Used for sending EBOOKCOIN to accounts
var accountSendTurn = 0;

function sendEbookcoin (account, i){
    node.onNewBlock(function(err) {
        var randomEbookcoin = node.randomizeEbookcoin();
        node.api.put('/transactions')
            ...
            .end(function (err, res) {
                ...
                console.log("Sending " + randomEbookcoin + " EBOOKCOIN to " + account.address);
                node.expect(res.body).to.have.property("success").to.be.true;
                if (res.body.success == true && i != null){
                    Accounts[i].balance = randomEbookcoin / node.normalizer;
                }
            });
    });
}

function sendEbookcoinfromMultisigAccount (amount, recipient){
    node.api.put('/transactions')
        .set('Accept', 'application/json')
        .send({
        ...
        .end(function (err, res) {
            console.log(JSON.stringify(res.body));
            console.log("Sending " + amount + " EBOOKCOIN to " + recipient);
            node.expect(res.body).to.have.property("success").to.be.true;
...
describe('Multisignatures', function() {
    ...
    before(function (done) {
        ...
        openAccount(NoEbookcoinAccount, null);
    ...
    before(function (done) {
       for (var i = 0; i < (Accounts.length); i++){
           if(Accounts[i] != null){
               sendEbookcoin(Accounts[i], i);
           }
       }
       sendEbookcoin(MultisigAccount, null);
       done();
    });
    ...
    describe('Create Multisignature Account', function() {
        ...
        test += 1;
        it(test + '. Account with 0 EBOOKCOIN. We expect error', function (done) {
            node.api.put('/multisignatures')
                .set('Accept', 'application/json')
                .send({
                    secret: NoEbookcoinAccount.password,
                    lifetime: 1,
                    min: requiredSignatures,
                    keysgroup: Keys
                })
